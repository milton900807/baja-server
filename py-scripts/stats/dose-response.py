...
def run_curvefit(settings_excel_file):
    ...
    print ('-----------------------------------',  settings_excel_file )
    settings, dff, df_samplenames = read_settings_file(settings_excel_file)
...
def calc_EC50(fn, dff, settings, t20):
    ...
    for sNum, sLet in enumerate(dfdose.index):
        ...
        if dfe.loc["n_resp_dp_below_yulc", sLet] < settings["min_num_dp_above&below_yulc"]:
            """The cells are "insuff_lowresp_dp" if any of the following are
            true.
        ...
        elif any([y_orig[1] < settings["min_resp_at_2nd_doseconc"], dfe.loc["n_resp_dp_above_yulc", sLet] < settings["min_num_dp_above&below_yulc"], np.array(y_orig)[-2] < settings["min_resp_at_2ndlast_doseconc"]]):
            """For high-throughput LD50 calculations, the cells have
            "insuff_highresp_dp" if any of the following are true:
            ...
            """
            for d in datasets:
                dfe.loc["EC50{}".format(d), sLet], dfe.loc["rsquared{}".format(d), sLet] = "insuff_highresp_dp", 00, False
                dfe.loc["residuals_mean{}".format(d),sLet], dfe.loc["y_fitted{}".format(d),sLet] = np.nan, np.nan
                dfe.loc["y_fitted_norm{}".format(d),sLet], dfe.loc["indices_lowdose_datapoints{}".format(d),sLet] = np.nan, np.nan
                dfe.loc["indices_lowdose_datapoints_excl_nearest_EC50{}".format(d),sLet] = np.nan
                dfe.loc["x_fitted{}".format(d), sLet], dfe.loc["response_lowdose_datapoints{}".format(d), sLet] = np.nan, np.nan
...
#         print ( method, ' --- and --- ', doseunits )
#         print ( af, ' --- and --- ', xaxis_left,yaxis_pos[3] )
#         print ( ' x y c ', xyc)
...
#         print ( ' \n\n --------------> ', maxAC )
    ...
    # set the axes array position
        ...
        print ( list(df_eval_values.loc[:,"EC50_calculable{}".format(d)]) )
            
        if True in list(df_eval_values.loc[:,"EC50_calculable{}".format(d)]):
            
            
            print (' Barchart01: barchart EC50 (full name on x-axis)   ' )
            #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
            #                                                                                                     #
...
def main(argv):
    print ( ' ----------------------------------- ')
    settings = "/workspaces/arcturing-server/sample-data/input.xlsx"    
    print ( ' args ' + sys.argv[1] )
    run_curvefit(sys.argv[1])
