...
export class DocTracker {
    ...
    updateAPI(app: any) {
        app.get('/test-doctrac', (req: any, res: any) => {
        ...
        app.get('/get-status', async (req: any, __res: any) => {
            ...
            db_client.connect(
                async err => {
                    if (err) {
                        ...
                        try {
                            const sql = `select signed -> 'signers' -> '${signer}' from exp.doctrac where id = '${dxpid}'`
        ...
        const setSigner = async (req: any, res: any) => {
            let id = '' + req.query.id;
            let signer = '' + req.query.signer;
            const signerName = '' + req.query.signerName;
         ...
         *  this assigns the envelope id to the user
         */
        app.get('/set-signer-status', setSigner)
        ...
        app.get('/remove-signer', async (req: any, res: { json: any }) => {
            let dxpid = '' + req.query.id;
            const signer = '' + req.query.signer;
            if (dxpid.endsWith('"')) {
                dxpid = dxpid.substring(1, dxpid.length - 1)
            ...
            db_client.connect(
                async err => {
                    if (err) {
                        ...
                        const sql1 = `update exp.doctrac set signed =  signed #- '{signers, ${signer}}'  where id = ${dxpid}`
                        const select_response = await db_client.query(sql1);
                        if (select_response == null || select_response.rowCount == 0) {
                            return res.json({ 'update': 'failed' });
                        }
                        const sql2 = `SELECT signed from exp.doctrac where id = ${dxpid}`
